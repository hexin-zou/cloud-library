...
export default {
  ...
  methods: {
    changeStatus(row) {
      ...
      request.put('/admin/admin/update', row).then(res => {
        if (res.code === '200') {
    ...
    savePass() {
      this.$refs['formRef'].validate((valid) => {
        if (valid) {
          request.put('/admin/admin/password', this.form).then(res => {
            if (res.code === '200') {
    ...
    del(id) {
      request.delete("/admin/admin/delete/" + id).then(res => {
        if (res.code === '200') {
