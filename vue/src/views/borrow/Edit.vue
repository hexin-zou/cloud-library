...
export default {
  ...
  created() {
    request.get('/book/book/list').then(res => {
      this.books = res.data
    })

    request.get('/user/user/list').then(res => {
      this.users = res.data.filter(v => v.status)
    })

    const id = this.$route.query.id
    request.get("/br/borrow/" + id).then(res => {
      this.form = res.data
    })
  },
  methods: {
    save() {
      request.put('/br/borrow/update', this.form).then(res => {
        if (res.code === '200') {
    ...
    selBook() {
      ...
      request.get('/book/book/' + book.id).then(res => {
