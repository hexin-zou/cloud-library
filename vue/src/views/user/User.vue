...
export default {
  ...
  methods: {
    changeStatus(row) {
      request.put('/user/user/update', row).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      request.delete("/user/user/delete/" + id).then(res => {
        if (res.code === '200') {
    ...
    addAccount() {
      this.$refs['ruleForm'].validate((valid) => {
        if (valid) {
          request.post('/user/user/account', this.form).then(res => {
            if (res.code === '200') {
